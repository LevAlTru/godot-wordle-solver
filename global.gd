...
func _ready() -> void:
	#var context := WordSearchContext.new().setGreen(0, "v").addYellow("v")
	#var results := getWords(context)
	#print(results.words)
	#print(results.lettersFound)
...
func isWorldValid(word: String) -> bool:
	return word.length() == 5

func isWordInTheDictionary(word: String) -> bool:
	return words.has(word.to_lower())

func formatInput(event: InputEvent) -> String:
	var st := event.as_text().split("+")[-1]
	if st == "Backspace": return "Backspace"
	if st == "Enter": return "Enter"
	var regex := RegEx.new()
	regex.compile("[A-Z]")
	if (st.length() == 1 and regex.search(st)):
		return st
	return ""
...
func getWords(context: WordSearchContext) -> WordSearchResult: # Magic function written in one sitting
	...
	if context.isEmpty(): 
		return result.setWords(words)
	var array: Array[String] = []
	
	var wordFailed := false
	...
	for word in words:
		...
		for i in range(5):
			...
			#else:
				#print(word[i] + "   " + greeny + "   ------------------")
			if lettersToFind.has(greeny): lettersToFind[greeny] = lettersToFind[greeny] + 1
			else: lettersToFind[greeny] = 1
		# Check yellow letters
		if not wordFailed:
			#print(word + " - green conditions are met")
			for let in word:
				if lettersToFind.has(let): 
					if lettersToFind[let] > 1: lettersToFind[let] = lettersToFind[let] - 1
					else: lettersToFind.erase(let)
				else:
					result.bufferIncreaseLetter(let)
			if lettersToFind.is_empty():
				result.confirmIncrease()
			else:
				result.confirmIncrease(false)
				wordFailed = true
				continue
		# Add if word passed all conditions
		if not wordFailed:
			#print(word + " - yellow conditions are met, the word can now pass")
			array.append(word)
